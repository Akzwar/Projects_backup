...
testApp::testApp()
{
    ...
    Rung = Integrator( M, 0.0,100, 10e-9, 10e-9);
...
void testApp::draw(){
   // ofClear(0,0,20);
	...
	glEnd();
        ...
        glBegin(GL_POINTS);
            glVertex2f(100+(Rung.getT()*Rung.getTk()*100),525-(double)Rung.CurrVect.U);
        glEnd();
	
	if(Rung.getT() < Rung.getTk() )
	printf("%2.10f,%2.20f\n",(double)(Rung.getT()*Rung.getTk()),
				(double)(Rung.CurrVect.U));
}
